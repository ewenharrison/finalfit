...
dim(colon_s)
colon_s = colon_s %>% 
	mutate(my_weights = runif(929, 0, 1))
...
colon_s %>%
	summary_factorlist(dependent, explanatory, p=TRUE, weights = "my_weights", total_col = TRUE, digits = c(1, 1, 3, 1, 1))


colon_s %>% 
	mutate(across(where(is.numeric) & any_of(explanatory), ~ .x * my_weights))
